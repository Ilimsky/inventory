...
class _TransferRightsDialogState extends State<_TransferRightsDialog> {
  ...
  List<Employee> get _employees => context.read<EmployeeProvider>().employees;

  List<Employee> get _filteredEmployees {
    if (_department == null) return _employees;
    final boundIds = context
        .read<BindingProvider>()
        .bindings
        .where((b) => b.departmentId == _department!.id)
        .map((b) => b.employeeId)
        .toSet();
    if (boundIds.isEmpty) return _employees;
    return _employees.where((e) => boundIds.contains(e.id)).toList();
  }
  ...
  Future<void> _printAct() async {
    ...
    try {
      final transferred = _previewSkeds.take(_result!.successCount).toList();
          ...
          child: AnimatedSwitcher(
            ...
            child: switch (_step) {
              ...
              _Step.preview => _PreviewStep(
                ...
                fromEmployee: _fromEmployee!,
                toEmployee: _toEmployee!,
                department: _department, // nullable
                reason: _reason,
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          // Филиал — временно закомментирован
          ...
          _EmployeeDropdown(
            ...
            validator: (v) => v == null ? 'Выберите передающего' : null,
          ),
          const SizedBox(height: 12),
          ...
          _EmployeeDropdown(
            ...
            validator: (v) => v == null ? 'Выберите принимающего' : null,
          ),
          const SizedBox(height: 12),
...
class _PreviewStep extends StatelessWidget {
  ...
  final Employee fromEmployee;
  final Employee toEmployee;
  final Department? department; // nullable — филиал временно отключён
  final TransferReason reason;
            ...
            _EmptyCard(employeeName: fromEmployee.name)
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: cs.error.withOpacity(.4), width: .8),
                ),
                child: Row(children: [
...
class _ResultStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (icon, color, title) = result.isFullSuccess
        ...
        ? (Icons.warning_amber_rounded, cs.tertiary, 'Частичная передача')
    ...
    return Column(
      ...
      children: [
        ...
        if (result.errors.isNotEmpty) ...[
          const SizedBox(height: 12),
          Text('Не удалось передать:', style: theme.textTheme.labelLarge),
        ...
        if (result.successCount > 0) ...[
          ...
          Row(children: [
            Icon(Icons.calendar_today_outlined,
                size: 14, color: theme.colorScheme.onSurface.withOpacity(.45)),
...
class _EmployeeDropdown extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final filtered = employees.where((e) => e.id != excludeId).toList();
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        return ListTile(
          ...
          title: Text(s.itemName, overflow: TextOverflow.ellipsis),
          subtitle: Text(
